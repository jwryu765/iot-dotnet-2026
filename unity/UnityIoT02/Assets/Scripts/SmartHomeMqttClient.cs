...
public class SmartHomeMqttClient : M2MqttUnityClient
{
    ...
    {
        brokerAddress = "192.168.0.41";
        brokerPort = 1883;
        autoConnect = true;

        base.Start();  // M2NqttUnityClient의 Start() 실행
    }
    ...
    protected override void SubscribeTopics()
    {
        ...
        status.gameObject.SetActive( true );  // 수신중 메세지 출력
        Debug.Log($"MQTT Subscribeed : {topic}");
    }

    protected override void UnsubscribeTopics()
    {
        // base.UnsubscribeTopics(); // 부모클래스 virtual 매서드에 아무것도 없음
        // 토픽 구독종료
    ...
    private void Update()
    {
        ...
        if (receivedMessages.Count > 0) // MQTT 메세지가 넘어왔으면
        {
