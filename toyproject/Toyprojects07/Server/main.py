...
## 내부 함수 2 - 같은 파일명 중복처리
def get_unique_filapath(directory: Path, filename: str):
...
def search_documents(question: str, top_k=2):
    # 질문을 Embegging으로 변환
...
async def upload(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 중복되지 않는 파일경로 생성
        save_path = get_unique_filapath(UPLOAD_DIR, safe_filename)
        ...
        # Chunk 단위로 나누기
        chunks = split_into_chunck(pages)
        # Embedding
